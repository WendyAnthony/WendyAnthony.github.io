...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(width = 3,
                 ...
                 br(),br(),
                 span(id = "input_panel",
                      selectInput("Menu1","Choose a Subregion", choices = c("All", unique(goe_eccc_sites_2$Subregion))),
                      ...
                      # selectInput("Menu3", "Choose a Variable", choices =  c("All", colnames(goe_eccc_sites_2)),
                      #            multiple = TRUE),
...
"),
                 ...
                 # Footer
                 br(),br(),
                 span(style = "font-size:10px; font-style:italic;", "Shiny App Showcase Example created by Wendy Anthony, modified 2025-09-17"),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Data Table", p(style = "font-size:11px;", "GOE Data from report tables: Malloff, J., & Shackelford, N. (2024). Feeling the Pulse: Monitoring methods and initial outcomes in oak meadow ecosystems. Restoration Futures Lab at the University of Victoria.
"),
...
"),
        ...
        tabPanel("Plot XY",
                 p(style = "font-size:11px;", "GOE Data from report tables: Malloff, J., & Shackelford, N. (2024). Feeling the Pulse: Monitoring methods and initial outcomes in oak meadow ecosystems. Restoration Futures Lab at the University of Victoria.
"),
                 screenshotButton(label = "Site Plot", id = "my_plot", filename = paste0("ECCC-Screenshot-Site-Plot-", format(Sys.time(), "%Y-%m-%d_%H.%M.%S")), download = FALSE, server_dir="."),
                 screenshotButton(label = "Subregion Plot", id = "my_plot1", filename = paste0("ECCC-Screenshot-Subregion-Plot-", format(Sys.time(), "%Y-%m-%d_%H.%M.%S")), download = FALSE, server_dir="."),
...
server <- function(input, output, session){
  ...
  output$text <- renderUI({
    ...
    str7d <- paste("Shiny App Proposal created by Wendy Anthony, modified 2025-09-15")
...
#    dat <- read.csv("data/goe_eccc_all_site_data.csv")
    dat <- data3()
    ...
    ggplot() +
      ...
      labs(title='Plotting Site Variables by Select Input',
           subtitle='ECCC GOE Monitoring Data',
           caption = "Chart by Wendy Anthony \n 2025-08-18",
