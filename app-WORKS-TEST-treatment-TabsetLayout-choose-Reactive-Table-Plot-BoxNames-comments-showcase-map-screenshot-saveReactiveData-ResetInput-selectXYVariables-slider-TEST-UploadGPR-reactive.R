...
grp_herbicide_data <- read.csv("data/OM_treatments_data_herbicide.csv", header = TRUE, sep = ",")
...
tabPanel("GRP Treatments",
         ...
         p(style = "font-size:11px;", "Hover over points to view tooltip ... if chart changes double-click within chart to return to original state"),
         screenshotButton(label = "GRP Fire Treatments", id = "plot_t_fire", filename = paste0("GRP-Screenshot-plot-fire-treatments", format(Sys.time(), "%Y-%m-%d_%H.%M.%S")), download = TRUE, server_dir="."),
         screenshotButton(label = "GRP Mowing Treatments", id = "plot_t_mowing", filename = paste0("GRP-Screenshot-plot-mowing-treatments", format(Sys.time(), "%Y-%m-%d_%H.%M.%S")), download = TRUE, server_dir="."),
         screenshotButton(label = "GRP Herbicide Treatments", id = "plot_t_herbicide", filename = paste0("GRP-Screenshot-plot-herbicide-treatments", format(Sys.time(), "%Y-%m-%d_%H.%M.%S")), download = TRUE, server_dir="."),
         plotlyOutput("plot_t_fire", width = "800px"),
         br(),hr(),br(),
         plotlyOutput("plot_t_mowing", width = "800px"),
         br(),hr(),br(),
         plotlyOutput("plot_t_herbicide", width = "800px"),
         ),
##########################################
# remove until code gets done for GRP
##########################################
...
server <- function(input, output, session){
  ...
  output$about_text <- renderUI({
    ...
    str4 <- paste("<i>(Shiny App created by Wendy Anthony 2025-09-25)</i>")
...
#   })
  ...
  output$plot_t_fire <- renderPlotly({
    ...
    grp_fire_data_seed <- grp_fire_data[grp_fire_data$restorationtype == "seeding", ]

    grp_fire_data_seed_plot <- ggplot(grp_fire_data_seed,
                                                # text = paste("Site:", Site),
    ...
    ## ggplotly
    ggplotly(tooltip = c("x", "y", "colour"), grp_fire_data_seed_plot) %>%
      config(displayModeBar = FALSE) %>% # This line disables the modebar
      layout(title = list(
  ...
  output$plot_t_mowing <- renderPlotly({
    ...
    ## ggplotly
    ggplotly(tooltip = c("x", "y", "colour"), grp_mowing_data_seed) %>%
      config(displayModeBar = FALSE) %>% # This line disables the modebar
      layout(title = list(
  ...
  output$plot_t_herbicide <- renderPlotly({
    ...
    grp_herbicide_data_seed <- grp_herbicide_data[grp_herbicide_data$restorationtype == "seeding", ]

    grp_herbicide_data_seed_plot <- ggplot(grp_herbicide_data_seed,
                                                     # text = paste("Site:", Site),
      aes(x = factor(siteid), y = factor(trt_year), color = treatmentid)) +
      geom_point(shape = 20, size = 1) +
      geom_jitter() +
    ...
    ## ggplotly
    ggplotly(tooltip = c("x", "y", "colour"), grp_herbicide_data_seed_plot) %>%
      config(displayModeBar = FALSE) %>% # This line disables the modebar
      layout(title = list(
