...
HTML("
  ...
  * Data Visualizations are responsive to data filters<br>
...
```{r filter-input-inv-biomass}
htmltools::br()
htmltools::em("Note: These filters need to be manually reset to delete unwanted choices")
htmltools::br()
htmltools::br()
h4("Filtered Statistics")
p(class = "container", class = "value", summarywidget(sd_bio_inv, column = "Crew_Hours", statistic = "sum"), span(class = "caption", "= hours of labor"))
p(class = "container", class = "value", summarywidget(sd_bio_inv, column = "Biomass_Removed_m3", statistic = "sum"), span(class = "caption", "= m3 of biomass"))
p(class = "container", class = "value",  summarywidget(sd_bio_inv, column = "Area_Cleared_m2", statistic = "sum"), span(class = "caption", "= m2 of area"))
htmltools::br()
htmltools::br()
# Create a filter input
filter_select(id = "Site", label = "Choose Site", sharedData = sd_bio_inv, group = ~ Site, multiple = T)
filter_checkbox(id = "Year", label = "Choose Year", sharedData = sd_bio_inv, group = ~ Year, allLevels = TRUE)
filter_select(id = "Invasive_Species_Cleared", label = "Choose Invasive Species", sharedData = sd_bio_inv, group = ~ Invasive_Species_Cleared, multiple = T)
htmltools::br()
htmltools::hr(style = "border: 0.5px solid grey; width: 250px; margin: auto;")
htmltools::br()
HTML("<img src='https://images.squarespace-cdn.com/content/v1/5e3c5b7e5460c55405a6d4d6/a8c2fb30-96fd-4042-925f-e76c7040dce6/Black+Logo+2.png?format=5w'>")
...
### Biomass m3 per Invasive Species
```{r inv-sp-biomass}
...
ggplotly(
  ggplot(sd_cover, aes(Species, mean_value, fill = Treated)) + 
  geom_bar(stat = "identity", width = 0.5, position = "dodge") +
  geom_errorbar(aes(ymax = mean_value + sd_value, ymin = pmax(mean_value - sd_value, 0)), width =.02, position = position_dodge(0.5)) +
  theme_minimal() + #get rid of grey background and tick marks
  theme(legend.position ="bottom") + 
  theme(axis.text.x = element_text(angle = 25, vjust = 1, hjust=1, size = 7)) +
    scale_fill_manual(values = custom_colours_2) +
        labs(
          title='Species Mean % Cover (& SD) by Treat',
          x = "", y = "Mean % Cover", fill = "")
)
```
