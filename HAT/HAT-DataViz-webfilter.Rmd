...
HTML("<strong>Created and Coded for HAT by Wendy Anthony 2026-01-19 05:44</strong>")
...
### **Jitter-Box Top 5 Species Biomass** 
...
### **Jitter-Box Invasive Grass Biomass** 
...
```{r filter-crew-hours-all-density}
#############################################
h4("Filtered Statistics")
#p(class = "container", class = "value", summarywidget(sd_crew_vol, column = "Crew_Hours", statistic = "sum"), span(class = "caption", "= hours of labor"))
p(class = "container", class = "value", summarywidget(sd_crew_vol, column = "Biomass_Removed_m3", statistic = "sum"), span(class = "caption", "= Biomass Removed (m3)"))
...
#############################################
htmltools::br()
htmltools::HTML("<strong>Data Source:</strong> <em>HAT Restoration Crew Data.xlsx and Matson Field Logs</em>")
htmltools::br()
htmltools::br()
htmltools::hr(style = "border: 0.5px solid grey; width: 250px; margin: auto;")
htmltools::br()
...
```{r}
htmltools::HTML("
  Density plots show the distribution of a specific variablem e.g. number of times the work hours or biomass removed equal a certain amount. They do not respond to filters for Year, or Species, or Work/Volunteer")
```
...
ggplotly(
  ggplot(sd_crew_vol, aes(x = Crew_Hours)) +
    ...
    labs(title = "Density plot of crew hours") +
  theme_minimal() 
  ) 
```
...
ggplotly(
  ggplot(sd_crew_vol, aes(x = Biomass_Removed_m3)) +
    geom_density(aes(fill = Site), alpha = 0.5) + 
    labs(title = "Density plot of Invasive Biomass Removed (m3)") +
  theme_minimal() 
  ) 
```
