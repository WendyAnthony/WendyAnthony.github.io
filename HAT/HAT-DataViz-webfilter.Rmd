...
# 
# Purpose: Interactive dashboard for visuzlizing GOE restoration monitoring data
# Data sources: 
...
```{r data-import-cover, include = FALSE}
# Load Data
################################# 
# Cover data
...
```{r data-import-species-count, include = FALSE}
# Load Data
################################# 
# Species counts
...
## Import spatial Data
OH_2024_ShinyG_Burning_Priorities <- st_read("data_spatial/OH_2024_ShinyG_Burning_Priorities/OH_2024_ShinyG_Burning_Priorities.shp")
OH_2024_Invasive_Species_Removed <- st_read("data_spatial/OH_2024_Invasive_Species_Removed/OH_2024_Invasive_Species_Removed.shp")
OH_2024_Identified_Invasive_Species_Patches <- st_read("data_spatial/OH_2024_Identified_Invasive_Species_Patches/OH_2024_Identified_InvasiveSpecies_Patches.shp")
OH_2024_Invasive_Species_Points <- st_read("data_spatial/OH_2024_Invasive_Species_Points/OH_2024_Invasive_Species_Points.shp")
OHP_2025_Completed_polygons <- st_read("data_spatial/OHP_2025_Completed_polygons/OHP_2025_Completed polygons.shp")
OHP_2025_Experimental_test_plots <- st_read("data_spatial/OHP_2025_Experimental_test_plots/OHP_2025_Experimental test plots.shp")
```
...
```{r spatial-subset-save, include = FALSE, cache = TRUE}
...
## THIS DOES NOT WORK !!!
# Spatial Data import
# rror in read.table(file = file, header = header, sep = sep, quote = quote,  : more columns than column names
#These won't read in
...
#############################################
sd_sites <- SharedData$new(data = HAT_sites_HAT, group = "crew_group")### Create sharable database using Crosstalk packa
...
Spatial data for Oak Haven shows polygons of restoration and species locations, but does not yet respond to filters.
...
#filter_select(id = "Site", label = "Choose a Site Filter", sharedData = sd_sites, group = ~Site)

# Municipality_filter <- filter_select(id = "Municipality", label = "Choose a Municipality Filter", sharedData = sd_sites, group = ~Municipality)
...
map_OH_sp <- leaflet() %>%
  ...
  addLayersControl(
    ...
    # adding colored icons in front of overlay group stopped toggle on/off ???
    # overlayGroups = unname(c("2025 Completed polygons <- <div style = 'position: relative; display: inline-block; width: 10px; height: 10px; background-color: #AA4499;'></div> 2025 Completed polygons", "<div style = 'position: relative; display: inline-block; width: 10px; height: 10px; background-color: #EECC66;'></div> 2025 Experimental test plot points", "<div style = 'position: relative; display: inline-block; width: 10px; height: 10px; background-color: #EE3377;'></div> 2024 Invasive Species Removed", "<div style = 'position: relative; display: inline-block; width: 10px; height: 10px; background-color: #33BBEE;'></div> 2024 Invasive Species Patches", "<div style = 'position: relative; display: inline-block; width: 10px; height: 10px; background-color: #EE7733;'></div> 2024 ShinyG Burning Priorities", "<div style = 'position: relative; display: inline-block; width: 10px; height: 10px; background-color: #CC3311;'></div> 2024 Invasive Species Points")),
    options = layersControlOptions(collapsed = TRUE) # Makes control box always visible
  ) %>%
  hideGroup(c("2024 ShinyG Burning Priorities", "2024 Invasive Species Removed", "2024 Invasive Species Patches", "2024 Invasive Species Points", "Spatial Polygon Color Legend")) %>%
  addScaleBar("topleft")  
# %>%
#       # add controls
