...
OH_spatial_points_alll_sub <- subset(OH_spatial_points_all, select = -c(1,4,5,6))
colnames(OH_spatial_points_alll_sub)
```
...
HTML("
  <br>This interactive data visualization tool was created by Wendy Anthony for HAT as a final project for University of Victoria Restoration of Natural Ecosystems ER390.
...
```{r}
htmltools::HTML("Interactive maps: Click map markers to open pop-up windows site-specific information. 
The three public HAT sites are listed on <a href='https://hat.bc.ca/community-based-restoration'>HAT's webpage</a>, all other HAT sites are on private, non-public land with location protected for privacy. The layer menu at the top right is used to toggle spatial data layers. 
The spatial data for Oak Haven shows polygons of restoration and species locations, but does Not yet respond to filters.
The measuring tool (under the zoom) can be used to measure distance between or within sites.
                ")
```
...
Site_filter <- filter_select(id = "Site", label = "Choose a Site Filter", sharedData = sd_sites, group = ~Site)
...
#   widths = c(1, 11),
#   "",
#   htmltools::strong("HAT GOE Monitoring Covenant 3-Sites Map")
# )

# bscols(widths = c(1,5,5,1),
#   "",
#   Site_filter, 
#   Municipality_filter,
#   "")
...
htmltools::br()
 datatable(OH_spatial_points_alll_sub, rownames = FALSE, class = 'compact',
           extensions = c("Buttons"),
           options = list(dom = 'Bfrtip',
                         buttons = list(                        
                        'colvis',
